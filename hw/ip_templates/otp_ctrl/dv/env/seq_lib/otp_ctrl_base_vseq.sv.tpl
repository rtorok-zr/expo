...
class otp_ctrl_base_vseq extends cip_base_vseq #(
  ...
  virtual task dai_wr(bit [TL_DW-1:0] addr,
    ...
    if (is_secret(addr) || is_zeroized_addr(addr) || is_sw_digest(addr))
    ...
    // TODO: An assertion check needs to be inserted for dai_idle when ever a valid dai_cmd is
    ...
    // TODO: direct_access_regwen prediction is also flawed in the scoreboard and needs to be fixed
  ...
  virtual task dai_rd(input  bit [TL_DW-1:0] addr,
    ...
    // When reading back after zeroization, when access errors are observed; idle check failures are
